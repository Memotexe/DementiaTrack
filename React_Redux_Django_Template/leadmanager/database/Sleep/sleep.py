...
from .majorrest import MajorRestPeriod
from .colekripke import ColeKripke
from .utils import (
    downsample,
    bin2df,
    high_pass_filter,
...
class SleepPy(object):
    ...
    *** Support for data from other wrist worn accelerometers will be added in the future. To maximize generality an
    ...
    def load(self):
        ...
        self.raw_days = [
            day[1]
            for day in df.groupby(pd.Grouper(level=0, freq="24h", offset=12))
    ...
    def export(self):
        ...
        export_df.to_csv(
            "{}_output_table.csv".format(self.src_name), index=False
        )
        return
